# 微信登录 & 企业认证 PRD（PC端 + 小程序端）v2.0

> **项目概括**：通过微信扫码/授权登录降低注册门槛，登录后引导实名认证（个人/企业），认证不强制但下单前必须完成，支持多端账号打通。
...
| v2.0 | 2026-05-06 | 精简结构，补充认证逻辑说明，重点突出 |
...
| 事项 | 说明 | 后果 |
|------|------|------|
| 微信开放平台绑定 | PC端网站应用 + 小程序必须绑定到**同一微信开放平台主体账号** | unionid不一致，多端无法打通 |
| openid + unionid 同时存储 | 登录时**必须同时获取并存储 openid + unionid** | 后上线端无法打通 |
| 授权回调域名配置 | PC端需在微信开放平台配置授权回调域 + HTTPS证书 | 二维码无法回调 |
| wechat_auth表结构 | 第一期必须完整建好（含platform、unionid字段） | 后期数据混乱 |

### 🔑 核心实现要点

| 模块 | 实现要点 |
|------|---------|
| **二维码生成** | 调用微信开放平台接口，5分钟有效期，过期刷新 |
| **登录状态轮询** | 前端每2秒轮询一次，服务端通过code换openid+unionid |
| **首次登录绑定** | unionid无对应账号 → 弹窗绑定手机号 → 关联用户 |
| **认证状态判断** | auth_status = none → 弹窗引导；personal/enterprise → 放行 |
| **下单前拦截** | 提交订单前检查auth_status，未认证弹窗拦截 |
| **多端打通** | unionid作为跨端唯一标识，查到则自动关联同一user_id |

### 📋 接口清单

| 接口 | 方法 | 说明 |
|------|------|------|
| /api/auth/wechat/qrcode | GET | 获取PC端微信扫码二维码 |
| /api/auth/wechat/callback | POST | 微信授权回调 |
| /api/auth/wechat/bind-phone | POST | 首次登录绑定手机号 |
| /api/auth/miniapp/login | POST | 小程序微信授权登录 |
...
- [ ] 小程序微信授权登录（老用户直接登录）
- [ ] 小程序微信授权登录（新用户绑定手机号）
- [ ] 登录后认证选择弹窗（个人/企业/暂不认证）
- [ ] 下单认证拦截（未认证用户被拦截）
- [ ] 同一微信号在PC端和小程序端登录到同一账号（多端打通）