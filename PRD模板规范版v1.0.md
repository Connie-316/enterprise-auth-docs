...
flowchart LR
    A[用户] --> B[认证入口]
    B --> C{后端判断<br/>auth_status}

    C -->|UNVERIFIED(0)| D[场景一：首次认证]
    C -->|其他状态| E[场景二：已认证授权]

    D --> F[填写表单<br/>含transactorInfo]
    E --> G[填写表单<br/>不含transactorInfo]

    F --> H[提交]
    G --> H

    H --> I[后端调接口1<br/>返回authUrl]
    I --> J[e签宝H5认证]

    J --> K{场景}

    K -->|场景一| L[人脸+营业执照<br/>+授权]
    K -->|场景二| M[仅授权]
...
| 版本 | 日期 | 修改内容 |
|------|------|---------|
| v1.0 | 2026-05-04 | 初始版本，按PRD模板规范重写 |
